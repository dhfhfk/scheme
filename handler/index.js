...
module.exports = async (client) => {
    ...
    adminCommands.map((value2) => {
        const file2 = require(value2);
        if (!file2?.name) return;
        client.slashCommands.set(file2.name, file2);

        if (["MESSAGE", "USER"].includes(file2.type)) delete file2.description;
        arrayOfAdminCommands.push(file2);
    });
