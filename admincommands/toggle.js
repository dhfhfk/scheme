            ...
            if (result.schedule.paused == false) {
                mongo().then(async (mongoose) => {
                    try {
                        ...
                        mongoose.connection.close();
                        console.log(`[👷] (${userId}, ${userName}) PAUSE schedule BY (관리자)`);
                        const paused = {
                            color: config.color.primary,
                            title: `관리자에 의해 스케줄이 일시정지 되었어요.`,
                            description: `\`/스케줄토글\` 명령어를 사용하면 일시정지를 해제할 수 있어요.`,
                            author: {
                        ...
                        client.channels.cache.get(channelId).send({
                            content: `<@${userId}> 관리자에 의해 스케줄 일시정지`,
                            embeds: [paused],
                        });
                ...
                mongo().then(async (mongoose) => {
                    try {
                        ...
                        mongoose.connection.close();
                        console.log(`[👷] (${userId}, ${userName}) UNPAUSE schedule BY (관리자)`);
                        const unpaused = {
                            color: config.color.primary,
